...
setwd('~/Google Drive/WashU/Phageome/SIG/Manuscript/Figures/GitHub/Figure2')

X_all   = read_excel("Data_S4_v1.xlsx", sheet = "defense_sys_sum")
X_data = as.data.frame.matrix(table(X_all$isolate, X_all$type))

Y_all   = read.csv("isolate_vOTU_matrix.csv", header = T)
Y_range = filter(Y_all, !grepl("SF_0046|SF_0070|SF_0126|SF_0255|SF_0256|SF_0257|SF_0258|SF_0260", Isolate)) # remove S. delphini
rownames(Y_range) = Y_range$Isolate
Y_data <- subset(Y_range, select = -c(Isolate, Cohort))
...
network(final.spls.liver, comp = 1:2,
        ...
        name.save = '250710_sPLS_phagePOP_defense_Network_0_cut0.05_v1')
...
cim(final.spls.liver, comp = 1:2, xlab = "Phage population", ylab = "Phage defense system", 
    ...
    save = 'pdf', # save as a png to the current working directory
    name.save = '250710_sPLS_phagePOP_defense_heatmap_0_v1')
